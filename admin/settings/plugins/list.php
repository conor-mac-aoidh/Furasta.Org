...
<img src="/_inc/img/plugins.png" style="float:left"/> <h1 class="image-left">Plugins</h1></span>
<br/>
...
foreach($Plugins->plugins as $plugin){
	...
	$status=(in_array($p_name,$PLUGINS))?'<a href="settings.php?page=plugins&action=deactivate&p_name='.$p_name.'">De-activate</a>':'<a href="settings.php?page=plugins&action=activate&p_name='.$p_name.'">Activate</a>';
        $content.='<tr>
                        <td class="small"><input type="checkbox" class="p-box" value="'.$p_name.'" name="trash-box"/></td>
                        <td class="first">'.$plugin->name.'</a></td>
                        <td>'.@$plugin->description.'</a></td>
			<td>'.$status.'</td>
                        <td><a href="settings.php?page=plugins&action=delete&p_name='.$p_name.'"<img src="/_inc/img/delete.png" title="Delete User" alt="Delete User"/></a></td>
                </tr>';
}
